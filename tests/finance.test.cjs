...
function harness() {
  ...
  const unexpected = () => {
    throw new Error("Unexpected UI initialization");
  };
  ...
  const put = (name, value) => {
    context[name] = structuredClone(value);
  };
...
test("summary adds cents exactly and counts income and expenses", () => {
  const h = harness();
  h.put("input", [
    transaction({ type: "income", amount: 0.1 }),
    transaction({ type: "income", amount: 0.2 }),
...
for (const [totalAmount, installments] of [
  [100, 3],
  [0.05, 2],
  [1234.57, 60],
]) {
  test(`installments preserve total cents: ${totalAmount} / ${installments}`, () => {
    ...
    assert.equal(
      cents.reduce((sum, amount) => sum + amount, 0),
...
test("legacy and v2 backups round-trip financial collections and metadata", () => {
  ...
  h.put("input", {
    ...
    budgets: { Mercado: 100 },
    recurring: [{ id: "rec-1", amount: 10, day: 3 }],
    installments: [plan()],
    goals: [{ id: "goal-1", target: 100, current: 10 }],
    archives: [
      {
        id: "arc-1",
        month: "2026-08",
        transactions: [transaction({ amount: 5, installmentNumber: 0, installmentTotal: 0 })],
      },
    ],
  });
...
test("date ranges override month, include endpoints and reject inverted ranges", () => {
  const h = harness();
  h.put("input", [
    transaction({ id: "aug", date: "2026-08-31" }),
    transaction(),
...
test("type, category and account filters compose", () => {
  ...
  h.put("input", [
    ...
    transaction({ id: "account", account: "Carteira" }),
    transaction({ id: "category", category: "Outros" }),
  ]);
  h.run("state.transactions = input");
...
test("all sort modes and amount ties preserve original state order", () => {
  const h = harness();
  h.put("input", [
    transaction({ id: "a", description: "Abacate", amount: 20, date: "2026-09-01" }),
...
test("normalization is idempotent for absent installment metadata", () => {
  ...
  assert.deepEqual(
    h.plain("normalizeTransaction(normalizeTransaction(input))"),
